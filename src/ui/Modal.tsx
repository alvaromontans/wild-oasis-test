...
function Open({ opens: openWindowModal, render: renderButton }: OpenProps) {
  ...
  const { open } = context;
  return renderButton!(() => open(openWindowModal!));
}
...
function Window({ name, render: renderForm }: WindowProps) {
  const context = useContext(ModalContext);
